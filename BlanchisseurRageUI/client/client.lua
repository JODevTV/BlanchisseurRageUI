...
local function openBlanchiement()
	RMenu.Add("zj_blanchi", "zj_blanchi_main", RageUI.CreateMenu("Blanchiment","Blanchir son argent"))
    ...
    if not MenuBalchie then 
		...
		Citizen.CreateThread(function()
			while MenuBalchie do
				RageUI.IsVisible(RMenu:Get("zj_blanchi",'zj_blanchi_main'),true,false,true,function()

					RageUI.Separator('Pourcentage : ~r~35%')

					RageUI.ButtonWithStyle("Blanchir votre argent", nil, {RightLabel = "→"}, true, function(_, _, s)
						if s then
    ...
    while true do
        ...
        for k,v in pairs(Blanchir.LOC) do
            if #(PlyCoord - v.position) < 1.5 then
				ESX.ShowHelpNotification("Appuyez sur ~b~[E]~s~ pour parler à Joe")
